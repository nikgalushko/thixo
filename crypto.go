...
// nolint:gosec // check potential problems
...
func derivePassword(counter uint32, passwordType, password, user, site string) string {
	...
	_ = binary.Write(&buffer, binary.BigEndian, uint32(len(user))) // skip error because the bytes.Buffer always returns nil
...
func buildCustomCertificate(b64cert, b64key string) (certificate, error) {
	crt := certificate{}
...
func decryptAES(password, crypt64 string) (string, error) {
	if crypt64 == "" {
		return "", nil
	}
